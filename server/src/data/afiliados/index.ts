...
import { Afiliado, Reingreso } from "../types";

export function afiliados() {
    const insertAfiliado = async (afiliadoData: Afiliado): Promise<void> => {
    ...
    const deleteAfiliado = async (cedula: string): Promise<void> => {
        const query = "DELETE FROM afiliados_frecuentes WHERE cedula = ?;"
        try {
            const [result, fields] = await pool.query<RowDataPacket[]>(query, cedula)
        } catch (err) {
            console.log(`An error has ocurred when searching for if there was any user with the same username before: ${err}`)
            throw err
        }
    ...
    const insertAfiliadoRetirado = async (Afiliado: Afiliado) => {
    ...
    const getAfiliadosRetirados = async (cedula: string) => {
        const query = `SELECT * FROM afiliados_retirados WHERE cedula = ?`
        try {
            const [result, fields] = await pool.query<RowDataPacket[]>(
                cedula === "*" ? "SELECT * FROM afiliados_retirados;" : query,
                cedula
            )
            if (result !== null) {
                const retirados: Afiliado[] = result.map((afiliado) => {
                    return {
                        expediente: afiliado.expediente,
                        nombre: afiliado.nombre,
                        apellido: afiliado.apellido,
    ...
    const deleteAfiliadoRetirado = async (cedula: number) => {
    ...
    const getReingresoData = async (): Promise<Reingreso[] | null> => {
        try {
            ...
            const [result, fields] = await pool.query<RowDataPacket[]>(query)
            if (result !== null) {
                const reingresos: Reingreso[] = result.map(row => {
                    return {
                        nombre: row.nombre,
