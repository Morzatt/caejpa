...
import type { Retiro, Afiliado, UserSession } from "../data/types";
import { retirosReport } from "./pdf";

export async function insertRetiro(req: Request, res: Response) {
    if (!rejectEmptyInput(req, res)) return

    const retiro: Retiro = req.body
...
export async function getRetiros(req: Request, res: Response) {
    if (!rejectEmptyInput(req, res)) return
            ...
            const filteredRetiros = retiros.filter((retiro) => {
                for (let key in retiro) {
                    if (typeof retiro[key] === "string") {
                        if (retiro[key].toLowerCase().includes(search)) return retiro 
                    } else if (typeof retiro[key] === "number") {
                        if (retiro[key].toString().toLowerCase().includes(search)) return retiro
                    }
                }
            })
...
export async function deleteRetiro(req: Request, res: Response) {
    if (!rejectEmptyInput(req, res)) return
    const id = req.params.id
...
export async function createRetirosReport(req: Request, res: Response) {
    const from = req.query.from as string
    const to = req.query.to as string
...
export async function sendRetiroStats(req: Request, res: Response) {
    try {
        const date = new Date()
        const resultTotalesAnterior = await retirosModel().getRetirosInfo(
            `${date.getFullYear()}-${date.getMonth()}-01`,
            `${date.getFullYear()}-${date.getMonth() + 1}-01`,
            "Retiro Total"
        )

        const resultParcialesAnterior = await retirosModel().getRetirosInfo(
            `${date.getFullYear()}-${date.getMonth()}-01`,
            `${date.getFullYear()}-${date.getMonth() + 1}-01`,
            "Retiro Parcial"
        )

        const resultTotales = await retirosModel().getRetirosInfo(
            `${date.getFullYear()}-${date.getMonth() + 1}-01`,
            `${date.getFullYear()}-${date.getMonth() + 2}-01`,
            "Retiro Total"
        )

        const resultParciales = await retirosModel().getRetirosInfo(
            `${date.getFullYear()}-${date.getMonth() + 1}-01`,
            `${date.getFullYear()}-${date.getMonth() + 2}-01`,
            "Retiro Parcial"
        )
