...
import { usuarios as usuariosModel } from "../data/usuarios"
...
export async function editUser(req: Request, res: Response) {
    ...
    try {
        const usuario = await usuariosModel().getUser(id)
        await usuariosModel().editUser(id, 
            (nombre ? nombre : usuario[0].nombre),
            (apellido ? apellido : usuario[0].apellido),
            (username ? username : usuario[0].username),
            (role ? role : usuario[0].role)
        )
        res.status(200).send({
