...
import type { User, PreguntasSeguridad } from "../data/types";
...
const preguntasDeSeguridad: string[] = [
...
// 
export async function sendSecurityQuestions(req: Request, res: Response) {
    if (!rejectEmptyInput(req, res)) return
...
// 
export async function saveSecurityQuestions(req: Request, res: Response) {
    if (!rejectEmptyInput(req, res)) return
    const usuario = await login().getUser(req.body.usuario)

    const preguntasDeSeguridad: PreguntasSeguridad = {
...
export async function getUserSecurityQuestions(req: Request, res: Response) {
    if (!rejectEmptyInput(req, res)) return
    ...
    try {
        ...
        if (DBuser !== null) {
            ...
            const securityQuestions: PreguntasSeguridad[] = await registerModel().getSecurityQuestions(usuario[0].id)
